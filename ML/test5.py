...
# Cellkoder: 0 = tom, 1 = vägg, 2 = ormdel, 3 = frukt
...
class SnakeGameAI:
    ...
    def play_step(self, action):
        ...
        Modifierad belöningsfunktion:
          - Beräknar Manhattan-avståndet från huvudet till maten före och efter rörelsen.
          - Om avståndet minskar ges en bonus, om det ökar straff, plus ett litet stegstraff.
          - Vid ältande av maten ges +10, vid kollision -10.
        """
        ...
        # Beräkna Manhattan-avståndet före rörelsen
        ...
        # Beräkna Manhattan-avståndet efter rörelsen
        ...
        if (self.head == self.food).all():
            ...
            # Belöna proportionellt mot hur mycket avståndet minskar och ge ett litet stegstraff
            reward = (prev_distance - new_distance) * 0.2 - 0.01
            self.snake.pop()

        self.update_ui()
        self.clock.tick(SPEED)

        # Timeout: om spelet varar för länge, avsluta episoden
...
def get_state_grid(game):
    ...
    # Placera ormen (värde 2)
    for s in game.snake:
        x, y = s
        if 0 < x < GRID_WIDTH-1 and 0 < y < GRID_HEIGHT-1:
            grid[y, x] = 2

    # Placera maten (värde 3)
...
def one_hot_state(grid):
    """
    One-hot-encodar gridet.
    Returnerar en tensor med shape (4, GRID_HEIGHT, GRID_WIDTH)
    där varje kanal representerar en celltyp.
    """
...
class Agent:
    def __init__(self):
        ...
        self.batch_size = 64
        # Använder CNN_QNet med 4 ingångskanaler och 3 utgångar
