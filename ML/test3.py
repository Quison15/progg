...
# Riktningar i en dictionary för enkel användning
directions = {
...
class SnakeGameAI:
    ...
    def __init__(self, w=WIDTH, h=HEIGHT):
        ...
        pygame.display.set_caption("Snake AI - Deep Q-Learning (Belöning för överlevnad)")
    ...
    def reset(self):
        ...
        # Räknare för antal steg sedan senaste mat
        self.steps_since_last_apple = 0
        self.game_over = False

    def spawn_food(self):
        while True:
            self.food = np.array([random.randint(0, self.w // BLOCK_SIZE - 1),
                                  random.randint(0, self.h // BLOCK_SIZE - 1)])
    ...
    def play_step(self, action):
        ...
        Belöningsfunktion:
          - Vid varje steg ges en liten bonus för att hålla sig vid liv, men endast under de första 50 stegen efter senaste mat.
          - Om antalet steg sedan senaste mat överskrider 50 ges inget bonus (eller ett litet straff om det blir väldigt högt).
          - Vid kollision ges -10, vid att äta maten ges +10.
        """
        ...
        if (self.head == self.food).all():
            ...
            reward = 10
            self.spawn_food()
            # Återställ stegräknaren vid äppelförbrukning
            self.steps_since_last_apple = 0
        else:
            # Belöna för att hålla sig vid liv upp till en viss punkt
            if self.steps_since_last_apple <= 50:
                alive_reward = 0.05  # bonus per steg
            elif self.steps_since_last_apple <= 100:
                alive_reward = 0.0   # ingen bonus
            else:
                alive_reward = -0.1  # straff om det dröjer för länge
            reward = alive_reward
            self.snake.pop()
        ...
        # Timeout för att undvika oändliga loopar (om ormen tar för lång tid)
    ...
    def _move(self, action):
        ...
        # Konvertera till tuples för entydiga jämförelser
...
def get_state(game):
    """
    Returnerar en 11-dimensionell vektor som representerar spelets tillstånd.
    Innehåller:
      - Fara rakt fram, höger, vänster (booleskt)
      - Nuvarande riktning (booleskt för varje riktning)
      - Matens position relativt ormens huvud (booleskt för vänster, höger, upp, ner)
    """
    head = game.head
