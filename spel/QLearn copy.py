...
class QLearn:
    ...
    def update_Qtable(self, discrete_state, new_discrete_state, action):
        ...
        # If the episode is not over, perform the standard Q-learning update
        if not self.done:
            # --- MODIFIED LINE ---
            # Correctly find the best Q-value from the next state over ALL possible actions
            max_future_q = np.max(self.q_table[new_discrete_state])
            
            # The update formula is mathematically the same, just using the corrected max_future_q
        ...
        # If the episode IS done and the goal was reached, set the Q-value to 0
        elif self.new_state[0] >= self.env.unwrapped.goal_position:
